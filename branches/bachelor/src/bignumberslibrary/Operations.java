...
import bignumberslibrary.fourier.ntt.IntCarryCRT;
import bignumberslibrary.fourier.ntt.ModularArithmetic;
import bignumberslibrary.fourier.ntt.NTT;
import util.Convert;
...
public class Operations {
    ...
    @Deprecated
    public static int[] mulIntFNT(int[] array1, int[] array2) {

        int length = NTT.getTransformLength(array1.length + array2.length);

        int[] result = new int[length];

        int[] resultByMod0 = NTT.convolute(array1, array2, length, 0),
              resultByMod1 = NTT.convolute(array1, array2, length, 1),
              resultByMod2 = NTT.convolute(array1, array2, length, 2);

        result = new IntCarryCRT(2).carryCRT(resultByMod0, resultByMod1, resultByMod2,length); //10 ???
